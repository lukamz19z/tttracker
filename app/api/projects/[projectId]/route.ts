      ...
      cookies: {
        ...
        setAll(cookiesToSet) {
          try {
            ...
            // Ignore cookie writes where unavailable.
          }
        },
      },
    },
...
export async function PATCH(
  ...
  try {
    ...
    const status =
      body.status?.trim() || "ongoing";

    const projectYear =
      Number(body.projectYear);

    const projectSequence =
      Number(body.projectSequence);
    ...
    if (
      ...
      await renameProjectSharePointFolders({
        ...
        tenderingDriveId:
          currentProject.sharepoint_tender_drive_id,

        tenderingFolderId:
          currentProject.sharepoint_tender_folder_id,
...
export async function DELETE(
  ...
  try {
    ...
    await deleteProjectSharePointFolders({
      ...
      tenderingDriveId:
        project.sharepoint_tender_drive_id,

      tenderingFolderId:
        project.sharepoint_tender_folder_id,
    });
