  ...
  async function handleSave() {
    ...
    try {
      ...
      if (role !== "admin" && role !== "editor") {
        alert("You do not have permission to add towers.");
        setSaving(false);
        return;
      }
      ...
      if (error || !data) {
        throw new Error(error?.message || "Failed to create tower");
      }

      router.push(`/project/${projectId}/tower/${data.id}`);
      router.refresh();
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="p-8 max-w-5xl space-y-6">
      <div className="flex justify-between items-start gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-bold">Add Tower</h1>
          <p className="text-slate-500 mt-1">
            Create a single tower manually for this project.
          </p>
        </div>
      ...
      <div className="bg-white border rounded-2xl p-6 space-y-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            These values go into extra data so the system stays adaptable.
          </p>
        </div>
      ...
      <div className="bg-white border rounded-2xl p-6 space-y-6">
        <div className="flex justify-between items-start gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Add any extra imported-style fields specific to this project.
            </p>
          </div>
