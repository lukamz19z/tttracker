...
export default function NewDailyDocketPage() {
  ...
  const totalAssemblyPercent = useMemo(() => {
    const total = progressRows.reduce(
      (sum, row) => sum + Number(row.assembled_qty || 0),
      0
    );
  ...
  const totalErectionPercent = useMemo(() => {
    const total = progressRows.reduce(
      (sum, row) => sum + Number(row.erected_qty || 0),
      0
    );
    return Math.min(total, 100);
  }, [progressRows]);
  ...
  async function handleSubmit() {
    ...
    const towerStatus =
      totalErectionPercent >= 100
        ? "Complete"
        : totalErectionPercent > 0
        ? "Erection Started"
        : totalAssemblyPercent > 0
        ? "Assembly Started"
        : "Not Started";
    ...
    <div className="p-8 max-w-6xl space-y-8">
      <div>
        ...
        <p className="text-slate-500 mt-1">
          Enter labour, section quantities, delays, and upload the scanned
          docket.
        </p>
      </div>
          ...
          {labourRows.map((row, index) => (
            ...
            >
              <Input
                ...
                onChange={(value) =>
                  updateLabourRow(index, "worker_name", value)
                }
              />
              <Input
                label="Time In"
              ...
              <Input
                ...
                onChange={(value) =>
                  updateLabourRow(index, "total_hours", value)
                }
              />
              <button
                type="button"
