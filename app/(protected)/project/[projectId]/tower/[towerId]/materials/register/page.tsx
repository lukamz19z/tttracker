/*
  ...
  - Bundle UUIDs are the relationship identity. Duplicate display references are
    allowed when their bundle sections differ (e.g. 1/6 Crossarms and 1/6 Bolts).
*/
...
function bundleIdentityKey(bundle: Bundle): string {
  return bundle.id ? `ID:${bundle.id}` : `UI:${bundle.ui_id}`;
}

function bundleReferenceSectionKey(bundleNo: unknown, section: unknown): string {
  return `${normaliseBundleKey(bundleNo)}__${normaliseSection(safeString(section, "General"))}`;
}
...
export default function MaterialsPage() {
  ...
  const bundleById = useMemo(() => {
    const map: Record<string, Bundle> = {};
    bundles.forEach((bundle) => {
      if (bundle.id) map[bundle.id] = bundle;
    });
    return map;
  }, [bundles]);

  function resolveBundleForMember(member: Member): Bundle | undefined {
    if (member.bundle_id && bundleById[member.bundle_id]) {
      return bundleById[member.bundle_id];
    }

    const candidates = bundlesByReference[normaliseBundleKey(member.bundle_reference)] || [];
    if (candidates.length === 1) return candidates[0];

    const segment = member.tower_segment.trim()
      ? normaliseSection(member.tower_segment)
      : "";

    if (segment) {
      const segmentMatches = candidates.filter(
        (bundle) => normaliseSection(bundle.section) === segment,
      );
  ...
  function referenceIsUnique(bundle: Bundle): boolean {
    return (bundlesByReference[normaliseBundleKey(bundle.bundle_no)] || []).length === 1;
  }

  function deliveredQty(bundle: Bundle): number {
    let total = 0;
    ...
    deliveries.forEach((delivery) => {
      delivery.tower_bundle_delivery_items.forEach((item) => {
        ...
        // Legacy delivery rows only had bundle_no. Use them only when that
        // display reference is unique on the tower; never guess across duplicates.
        if (
          !item.bundle_id &&
          referenceIsUnique(bundle) &&
  ...
  const bundleCheckByBundleId = useMemo(() => {
    const map: Record<string, BundleCheck> = {};

    bundleChecks.forEach((check) => {
      if (check.bundle_id) {
        map[check.bundle_id] = check;
        return;
      }

      // Backward compatibility for old checks: only attach by bundle_no when
      // exactly one bundle currently uses that display reference.
      const candidates = bundlesByReference[normaliseBundleKey(check.bundle_no)] || [];
      if (candidates.length === 1 && candidates[0].id) {
        map[candidates[0].id!] = check;
      }
    });
  ...
  function getBundleCheck(bundle: Bundle): BundleCheck | undefined {
    return bundle.id ? bundleCheckByBundleId[bundle.id] : undefined;
  }

  function receivedQty(bundle: Bundle): number {
    return Math.max(safeNumber(getBundleCheck(bundle)?.qty_received, 0), 0);
  }

  function remainingDeliveryQty(bundle: Bundle): number {
    return Math.max(bundle.qty_required - deliveredQty(bundle), 0);
  }

  function remainingReceiveQty(bundle: Bundle): number {
    return Math.max(bundle.qty_required - receivedQty(bundle), 0);
  }

  function overReceivedQty(bundle: Bundle): number {
    return Math.max(receivedQty(bundle) - bundle.qty_required, 0);
  }
  ...
  const membersByBundleId = useMemo(() => {
    ...
    members.forEach((member) => {
      ...
      if (!bundle?.id) return;
      if (!map[bundle.id]) map[bundle.id] = [];
      map[bundle.id].push(member);
    });

    return map;
    // bundleById and bundlesByReference are derived from bundles and are read
    // through resolveBundleForMember.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [members, bundles]);

  function membersForBundle(bundle: Bundle): Member[] {
    return bundle.id ? membersByBundleId[bundle.id] || [] : [];
  }

  function memberLinesForBundle(bundle: Bundle): number {
    return membersForBundle(bundle).length;
  }

  function memberQtyFromMemberList(bundle: Bundle): number {
    return membersForBundle(bundle).reduce(
  ...
  const memberCheckMap = useMemo(() => {
    ...
    memberChecks.forEach((check) => {
      ...
      map[`${bundleId}__${check.mark_no.trim().toUpperCase()}`] = check;
    });
  ...
  const matchedMembersForBoltSections = useMemo(() => {
    return members.filter((member) => !!resolveBundleForMember(member));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [members, bundles]);
  ...
  function getMemberCheck(member: Member): MemberCheck | undefined {
    ...
    return memberCheckMap[`${bundle.id}__${member.mark_no.trim().toUpperCase()}`];
  }
  ...
  function deriveBundleStatus(bundle: Bundle): BundleCheckStatus {
    ...
    const relatedMembers = membersForBundle(bundle);
  ...
  function bundleMatchesStatus(bundle: Bundle, filter: StatusFilter): boolean {
    if (filter === "all") return true;
    return deriveBundleStatus(bundle) === filter;
  }
  ...
  function getOutstandingReason(bundle: Bundle, status: BundleCheckStatus): string {
    ...
    const delivered = deliveredQty(bundle);
  ...
  async function persistBundles(rows: Bundle[]) {
    ...
    if (payload.length > 0) {
      const { error } = await supabase.from("tower_required_bundles").upsert(payload, {
        onConflict: "tower_id,bundle_no,section",
      });
  ...
  async function persistMembers(rows: Member[]) {
    const payload = rows
      ...
      .map((row) => {
        const matchedBundle = resolveBundleForMember(row);

        return {
          tower_id: towerId,
          bundle_id: matchedBundle?.id || row.bundle_id || null,
    ...
    if (payload.length > 0) {
      const { error } = await supabase.from("tower_material_members").upsert(payload, {
        onConflict: "tower_id,bundle_reference,mark_no,tower_segment",
      });
    ...
    for (const row of selected) {
      ...
      if (row.bundle_no.trim() !== "") {
        const { error } = await supabase
          ...
          .eq("section", normaliseSection(row.section));
    ...
    for (const row of selected) {
      ...
      if (row.mark_no.trim() !== "") {
        const { error } = await supabase
          ...
          .eq("tower_segment", row.tower_segment.trim() ? normaliseSection(row.tower_segment) : "General");
  ...
  async function upsertBundleQtyReceived(
    bundle: Bundle,
    nextQty: number,
    forcedStatus?: BundleCheckStatus,
  ) {
    if (!bundle.id) {
      alert("Save this bundle before recording a site check.");
      return;
    }

    const required = Math.max(bundle.qty_required, 1);
    // Do not cap at required: over-received bundles are valid evidence and must
    // remain visible as excess rather than being silently forced back to required.
    const nextReceived = Math.max(Math.round(nextQty), 0);
    const autoStatus = forcedStatus || deriveStatusFromReceived(nextReceived, required);
    ...
    setBundleChecks((prev) => {
      const next = prev.filter((row) => row.bundle_id !== bundle.id);
      next.push(payload);
      return next;
    });
  }
  ...
  async function incrementBundleReceived(bundle: Bundle, amount: number) {
    await upsertBundleQtyReceived(bundle, receivedQty(bundle) + amount);
  }
  ...
  async function markBundleIssue(bundle: Bundle) {
    await upsertBundleQtyReceived(bundle, receivedQty(bundle), "issue");
  }
  ...
  async function updateMemberStatus(member: Member, status: MemberCheckStatus, notes?: string) {
    ...
    if (!bundle?.id) {
      alert(
        `TTTracker cannot safely identify the bundle for member ${member.mark_no}. ` +
          "Set the member's Tower Segment so it matches the correct duplicate bundle first.",
      );
      return;
    }
    ...
    setMemberChecks((prev) => {
      const key = `${payload.bundle_id}__${payload.mark_no.toUpperCase()}`;
  ...
  async function clearMemberStatus(member: Member) {
    const bundle = resolveBundleForMember(member);
    if (!bundle?.id) {
      alert("TTTracker cannot safely identify this member's bundle.");
      return;
    }
      ...
      prev.filter(
        (row) =>
          !(
            ...
            row.mark_no.trim().toUpperCase() === member.mark_no.trim().toUpperCase()
          ),
      ),
    );
  }

  async function clearWholeBundleStatuses(bundle: Bundle) {
    if (!bundle.id) {
      alert("Save this bundle before clearing its site checks.");
      return;
    }
    ...
    Papa.parse<CsvRow>(file, {
      ...
      complete: async (res: ParseResult<CsvRow>) => {
        ...
        parsedRows.forEach((row) => {
          const key = `${row.tower_id}__${normaliseBundleKey(row.bundle_no)}__${normaliseSection(row.section)}`;
          rowMap.set(key, row);
        });
...
const { error } = await supabase.from("tower_required_bundles").upsert(rows, {
  ...
  async function importMembersFile(file: File) {
    ...
    try {
      ...
      const bundlesByRef = new Map<string, Bundle[]>();
      bundles.forEach((bundle) => {
        const key = normaliseBundleKey(bundle.bundle_no);
        bundlesByRef.set(key, [...(bundlesByRef.get(key) || []), bundle]);
      });
      ...
      const selectionByMark = new Map<
        string,
        { selected: string[]; rejectedTower: string[]; rejectedBundle: string[]; ambiguous: string[] }
      >();
      ...
      sourceRows.forEach((r) => {
        ...
        const towerSegment = towerSegmentRaw ? normaliseSection(towerSegmentRaw) : "General";

        const candidateBundles =
          bundlesByRef.get(normaliseBundleKey(bundleReference)) || [];
        ...
        if (candidateBundles.length === 1) {
          matchedBundle = candidateBundles[0];
        } else {
          const segmentMatches = candidateBundles.filter(
            (bundle) => normaliseSection(bundle.section) === towerSegment,
          );

          if (segmentMatches.length === 1) {
            matchedBundle = segmentMatches[0];
          }
        }

        if (!matchedBundle?.id) {
          ambiguousBundleRows += 1;
          trace.ambiguous.push(
            `${bundleReference} (${towerSegment || "no segment"})`,
          );
          return;
        }
        ...
        const key = `${matchedBundle.id}__${markKey}`;
        ...
        trace.selected.push(`${bundleReference} · ${matchedBundle.section}`);
      });
      ...
      if (!rows.length) {
        alert(
          [
            ...
            `Required bundle records: ${bundles.length}`,
            ...
            `Ambiguous duplicate bundle rows: ${ambiguousBundleRows}`,
      ...
      const { data: savedRows, error } = await supabase
        ...
        .select("tower_id,bundle_id,bundle_reference,mark_no,tower_segment");
      ...
      const matchedBundleCount = new Set(rows.map((row) => row.bundle_id)).size;
      ...
      const duplicateMarks = Array.from(selectionByMark.entries())
        .filter(([, trace]) =>
          trace.selected.length +
            trace.rejectedTower.length +
            trace.rejectedBundle.length +
            trace.ambiguous.length >
          1,
        )
        .slice(0, 5)
        .map(([mark, trace]) => {
          const selected = Array.from(new Set(trace.selected)).join(", ") || "none";
          const ambiguous = Array.from(new Set(trace.ambiguous)).join(", ");
          return `${mark}: selected ${selected}${ambiguous ? ` · ambiguous ${ambiguous}` : ""}`;
        });
      ...
      alert(
        [
          ...
          `Bundle UUIDs matched: ${matchedBundleCount}/${bundles.length}`,
          `Tower-specific rows matched: ${restrictedRowsMatched}/${restrictedRowsSeen}`,
          `Other-tower rows ignored: ${otherTowerRows}`,
          `Other-bundle rows ignored: ${otherBundleRows}`,
          `Ambiguous duplicate-bundle rows ignored: ${ambiguousBundleRows}`,
...
const { error } = await supabase.from("tower_material_bolts").upsert(rows, {
  ...
  const filteredBundles = (() => {
    ...
    return bundles.filter((bundle) => {
      ...
      if (!bundleMatchesStatus(bundle, statusFilter)) return false;
      ...
      const text = matchesText(
        ...
        memberLinesForBundle(bundle),
        memberQtyFromMemberList(bundle),
        ...((membersForBundle(bundle)).map((m) =>
          [
            m.mark_no,
            m.drawing_number,
            m.section,
  ...
  const unmatchedMembers = useMemo(
    () => members.filter((member) => !resolveBundleForMember(member)),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [members, bundles],
  );
  ...
  const legacyAmbiguousDeliveryRefs = useMemo(() => {
    const duplicateRefs = new Set(
      Object.entries(bundlesByReference)
    ...
    deliveries.forEach((delivery) => {
      delivery.tower_bundle_delivery_items.forEach((item) => {
        const ref = normaliseBundleKey(item.bundle_no);
        if (!item.bundle_id && duplicateRefs.has(ref) && safeNumber(item.qty_delivered, 0) > 0) {
          refs.add(item.bundle_no);
        }
      });
    });

    return Array.from(refs).sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
  }, [deliveries, bundlesByReference]);
    ...
    if (viewMode === "bundles") {
      const rows = [
        ...
        ...filteredBundles.map((bundle) => [
          ...
          bundle.member_qty,
          bundle.total_weight ?? "",
          statusLabel(deriveBundleStatus(bundle)),
        ]),
      ];
...
>
  ...
  <option value="merge">Add / merge only</option>
            ...
            {legacyAmbiguousDeliveryRefs.length > 0 && (
              <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                <span className="font-black">Legacy delivery rows need bundle UUIDs:</span>{" "}
                {legacyAmbiguousDeliveryRefs.join(", ")}. TTTracker is deliberately not assigning those old
                bundle-number-only quantities across duplicate bundle references.
              </div>
            )}
            ...
            <div className="space-y-2">
              {filteredBundles.length === 0 ? (
                ...
                filteredBundles.map((bundle) => {
                  const relatedMembers = membersForBundle(bundle);
                  const status = deriveBundleStatus(bundle);
                  const expanded = expandedBundleKey === bundleIdentityKey(bundle);
                  const hasMemberStatuses = relatedMembers.some((member) => !!getMemberCheck(member));
                  const hasBundleStatus = !!getBundleCheck(bundle);
                  const delivered = deliveredQty(bundle);
                  const received = receivedQty(bundle);
                  const remainingToReceive = remainingReceiveQty(bundle);
                          ...
                          <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-2">
                            ...
                            <div className="rounded-xl border border-slate-200 bg-slate-50 p-2 min-w-[220px]">
                              ...
                              <div className="grid grid-cols-4 gap-1.5 mt-2">
                                ...
                                <button
                                  onClick={() =>
                                    setExpandedBundleKey((prev) => {
                                      const key = bundleIdentityKey(bundle);
                              ...
                              {(hasMemberStatuses || hasBundleStatus) && (
                                <button
                                  onClick={() => void clearWholeBundleStatuses(bundle)}
          ...
          {viewMode === "members" && (
            <div className="space-y-2">
              {filteredMatchedMembers.length === 0 ? (
                ...
                filteredMatchedMembers.map((member) => {
                  const status = getMemberCheck(member)?.status || "not_checked";
                  const matchingBundle = resolveBundleForMember(member);
                          ...
                          <div className="flex items-start gap-2 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="text-xs md:text-sm text-slate-500 mt-1 leading-5">
                                ...
                                {matchingBundle ? receivedQty(matchingBundle) : "—"} • Drawing{" "}
