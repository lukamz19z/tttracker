...
function isClosedFleetJobStatus(value: string | null | undefined) {
...
function getLatestUpdate(
  ...
  const allowedTypes = updateTypes.map((type) => type.toLowerCase());
    ...
    updates
      .filter(
        (update) =>
          ...
          allowedTypes.includes(clean(update.update_type).toLowerCase()),
      )
      .sort((a, b) => {
...
function getLatestAssetHistory(
  ...
  return (
    assetHistory
      .filter((record) => record.fleet_job_id === fleetJobId)
      .sort((a, b) => {
        const aTime = new Date(
          a.created_at || a.history_date || "1900-01-01",
        ).getTime();
        const bTime = new Date(
          b.created_at || b.history_date || "1900-01-01",
        ).getTime();
        return bTime - aTime;
      })[0] ?? null
  );
...
function calculateLiveStatus(
  ...
  const statusFromColumn = toFleetJobStatus(job.status);
  const closeOutUpdate = getLatestUpdate(updates, job.id, [
    "Close Out",
    "Close Out Edited",
  ]);
  const reopenUpdate = getLatestUpdate(updates, job.id, ["Reopened"]);
  const linkedAssetHistory = getLatestAssetHistory(assetHistory, job.id);

  const closeOutProofTime = Math.max(
    timeValue(job.completed_date),
    timeValue(closeOutUpdate?.created_at),
    timeValue(linkedAssetHistory?.created_at),
    timeValue(linkedAssetHistory?.history_date),
  );

  const reopenTime = timeValue(reopenUpdate?.created_at);
  const reopenedAfterCloseOut = reopenTime > 0 && reopenTime > closeOutProofTime;

  if (reopenedAfterCloseOut && !isClosedFleetJobStatus(job.status)) {
    return "Open";
  }

  if (isClosedFleetJobStatus(job.status) || closeOutProofTime > 0) {
    return "Completed";
  }
...
export default function FleetJobsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [
      ...
      supabase
        .from("asset_history")
        .select("id, fleet_job_id, history_type, history_date, created_at")
    ...
    if (assetHistoryResult.error) {
      console.error(
        ...
        assetHistoryResult.error.message,
      );
      setAssetHistory([]);
    } else {
      setAssetHistory((assetHistoryResult.data ?? []) as AssetHistory[]);
    }

    if (fleetJobUpdatesResult.error) {
      console.error(
        "Failed to load fleet job updates:",
        fleetJobUpdatesResult.error.message,
      );
      setFleetJobUpdates([]);
    } else {
      setFleetJobUpdates(
        (fleetJobUpdatesResult.data ?? []) as FleetJobUpdate[],
      );
    }
  ...
  const enhancedJobs = useMemo<EnhancedFleetJob[]>(() => {
    return jobs.map((job) => {
      ...
      const calculated_status = calculateLiveStatus(
        job,
        assetHistory,
        fleetJobUpdates,
      );
  ...
  }, [jobs, vehicleMap, plantMap, assetHistory, fleetJobUpdates]);
