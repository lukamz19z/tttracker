...
function isClosedJob(
  ...
  const latestCloseOutProofDate =
    closeOutUpdate?.created_at ||
    assetHistoryRows
      .filter((record) => record.fleet_job_id === job.id)
      .map((record) => record.history_date || record.created_at || "")
      .filter(Boolean)
      .sort()
      .at(-1) ||
    null;
  ...
  const reopenedAfterCloseOut = Boolean(
    reopenUpdate?.created_at &&
      latestCloseOutProofDate &&
      new Date(reopenUpdate.created_at).getTime() >
        new Date(latestCloseOutProofDate).getTime(),
  );

  if (reopenedAfterCloseOut) return false;
  if (statusClosed) return true;
  if (closeOutUpdate || hasLinkedAssetHistory) return true;
...
export default function CompliancePage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [
      ...
      safeSelect<AssetHistory>(supabase.from("asset_history").select("*")),
      safeSelect<FleetJobUpdate>(supabase.from("fleet_job_updates").select("*").order("created_at", { ascending: false })),
  ...
  const kpis = [
    ...
    { label: "Open Fleet Jobs", value: data.fleetJobs.filter((job) => !isClosedJob(job, data.assetHistory, data.fleetJobUpdates)).length, icon: Wrench, tone: "blue" as Tone, detail: "Fleet defects and maintenance not closed" },
  ];
