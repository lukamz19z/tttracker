...
export default function UpdateVehiclePage() {
  ...
  const loadPageData = useCallback(async () => {
    ...
    if (vehicleResult.error || !vehicleResult.data) {
      ...
      setForm((current) => ({
        ...
        service_interval_km:
          data.service_interval_km === null || data.service_interval_km === undefined
            ? "10000"
            : String(data.service_interval_km),
  ...
  const serviceIntervalKm = useMemo(() => {
    return toNumber(form.service_interval_km) ?? vehicle?.service_interval_km ?? 10000;
  ...
  const calculatedNextServiceKm = useMemo(() => {
    ...
    if (currentKm !== null) return currentKm + serviceIntervalKm;

    return null;
  }, [form.odometer_km, form.next_service_km, serviceIntervalKm]);
  ...
  async function saveServiceOrModificationHistory() {
    ...
    const serviceIntervalKmValue = isTrailer ? null : serviceIntervalKm;
    ...
    const historyPayload = isModification
      ...
      : {
          ...
          service_interval_km: serviceIntervalKmValue,
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const vehicleUpdatePayload = {
        ...
        service_interval_km: isTrailer ? null : serviceIntervalKm,
        next_inspection_due: isTrailer ? form.next_inspection_due || null : null,
