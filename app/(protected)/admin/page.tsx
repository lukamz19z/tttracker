...
export default function AdminPage() {
  ...
  const checkAccess = useCallback(async () => {
    ...
    if (!response.ok) {
      throw new Error(payload.error ?? "Could not verify access.");
    }
  ...
  useEffect(() => {
    void checkAccess()
      .catch((err) =>
        setError(
          err instanceof Error ? err.message : "Could not verify access.",
        ),
      )
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
            Manage login accounts, dynamic roles, user permissions, project
            access, SharePoint access and document branding.
          </p>
        </section>
        ...
        {tab === "accounts" ? (
          <AdminAccountsPanel onOpenAccess={() => setTab("permissions")} />
        ) : tab === "permissions" ? (
