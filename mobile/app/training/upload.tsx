...
export default function TrainingUploadScreen() {
  ...
  const [trainingTypeId, setTrainingTypeId] =
    useState(
      String(params.trainingTypeId ?? ""),
    );
  ...
  const [singleFile, setSingleFile] =
    useState<PickedTrainingFile | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (changesRecord) {
        setTrainingTypeId(
          changesRecord.training_type_id ??
            "",
        );
        setSelectedOptionIds(
          changesRecord.option_ids ?? [],
        );
        setProjectId(
          changesRecord.project_id ?? "",
        );
        setIssuer(
          changesRecord.provider ??
            changesRecord.issuing_authority ??
            "",
        );
        setCertificateNumber(
          changesRecord.certificate_number ??
            "",
        );
        setIssueDate(
          changesRecord.issue_date ?? "",
        );
        setExpiryDate(
          changesRecord.expiry_date ?? "",
        );
        setNotes(
          changesRecord.notes ?? "",
        );
        setMetadata(
          changesRecord.metadata ?? {},
        );
      }
    } catch (error) {
      setMessage(
        error instanceof Error
          ? error.message
          : "Training configuration could not be loaded.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const explicitReplaceId = String(
    params.replaceRecordId ?? "",
  );
  const changesRecordId = String(
    params.changesRecordId ?? "",
  );
  ...
  const replacementId =
    ...
    (selectedType
      ...
      false &&
    approvedCurrentRecords.length === 1
      ? approvedCurrentRecords[0]?.id
      : "") ??
    "";

  function validate(type: TrainingType) {
    if (!data?.employee?.id) {
      return "Your login is not linked to an employee profile.";
    }

    if (
      type.requires_project &&
      !projectId
    ) {
    ...
    if (type.requires_document) {
      if (
        ...
        return "Add both the front and back evidence.";
      }
      ...
      if (
        ...
        return "Add the required certificate / licence evidence.";
      }
    }
  ...
  async function submit() {
    ...
    try {
        ...
        await uploadTraining({
          ...
          documentUploadType:
            ...
            (selectedType.requires_document
              ? "single"
              : "none"),
          replacementMode: replacementId
            ? "replace"
            : approvedCurrentRecords.length >
                  0 &&
      ...
      if (
        ...
        await completeChangesRequiredResubmission(
          changesRecordId,
          result.recordId,
        );
      }
  ...
  return (
    <>
      <Stack.Screen
        options={{
          headerShown: true,
          title: changesRecordId
            ? "Fix Training Submission"
            : "Upload Training",
        }}
      />
      ...
      >
        ...
        <Card
          title="1. Training Type"
          hint="This configuration is shared with TTTracker web."
        >
          <Pressable
            style={styles.select}
            onPress={() =>
        ...
        {selectedType ? (
          <>
            ...
            <Card title="3. Details">
              ...
              "never" ? (
                <Text style={styles.helper}>
                  This Training type is configured
                  to not expire.
                </Text>
              ) : null}
            ...
            {selectedType.requires_document ? (
              <Card
                title="4. Evidence"
                hint="Preview the exact PDF or image before submitting. Evidence stays in TTTracker staging until approved, then is published to SharePoint."
              >
                {selectedType.document_upload_type ===
                "front_back" ? (
                  <>
            ...
            {replacementId ? (
              <Text style={styles.helper}>
                This submission will replace the
                existing approved record only after
                the new evidence is approved.
              </Text>
            ) : null}
...
function FileField({
  ...
  label: string;
  value: PickedTrainingFile | null;
  onChange: (
    value: PickedTrainingFile | null,
  ) => void;
  onPreview: (
    value: PickedTrainingFile,
  ) => void;
}) {
  async function camera() {
    ...
    if (!permission.granted) {
      Alert.alert(
        ...
        "Allow camera access to photograph Training evidence.",
      );
      return;
    }
