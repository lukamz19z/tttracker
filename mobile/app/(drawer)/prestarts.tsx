...
function dateForDb(value: string, label = "Date"): string {
  const date = parseDateValue(value);
  if (!date) {
...
export default function PrestartsScreen() {
  const { profile } = useAuth();
  const { can } = useAccess();
  const canVehiclePrestart = can("mobile.vehicle_prestarts");
  const canPlantPrestart = can("mobile.plant_prestarts");
  ...
  const loadData = useCallback(async (showLoader = true) => {
    ...
    const filtered = resolvedInspectorName
      ? allPrestarts.filter((prestart) => {
          ...
          if (assetType === "Vehicle" && !canVehiclePrestart) return false;
          if (assetType === "Plant" && !canPlantPrestart) return false;
  ...
  const formSeverity = useMemo(() => {if (failedItems.length === 0) return "none";
  ...
  function validateDetails(): boolean {
    ...
    if (!parseDateValue(prestartDate)) {
      Alert.alert(
        "Invalid date",
        "Enter the prestart date as DD/MM/YYYY.",
      );
      return false;
    }
  ...
  async function createFleetJob(
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        assigned_to: null,
        vendor: null,
        reported_date: dateForDb(prestartDate, "Prestart Date"),
        due_date: null,
        completed_date: null,
        cost: null,
  ...
  async function savePrestart() {
    ...
    try {
      ...
      const payload = {
        ...
        prestart_date: dateForDb(prestartDate, "Prestart Date"),
        kilometres:
          selectedAssetType === "Vehicle"
          ...
          return (
            <Pressable
              ...
              style={[
                ...
                active &&
                  option.value === "no" &&styles.answerButtonNo,
                active &&
                  option.value === "na" &&
