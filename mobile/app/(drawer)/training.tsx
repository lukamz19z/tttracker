...
function parseDateOnly(value: string | null | undefined) {
  ...
  // Database/API date values: YYYY-MM-DD or an ISO timestamp beginning with it.
  ...
  // App-facing format: DD-MM-YYYY or DD/MM/YYYY.
...
function status(record: TrainingRecord) {
  ...
  if (record.workflow_status === "changes_required") {
    return { key: "changes", label: "Changes Required", tone: "orange" };
  }
  ...
  if (record.revoked_at || record.record_status === "revoked") {
    return { key: "rejected", label: "Revoked", tone: "red" };
  }

  if (record.superseded_at || record.current_version === false) {
    return { key: "history", label: "Superseded", tone: "slate" };
  }
  ...
  const summary = useMemo(() => {
    ...
    return {
      ...
      issues: rows.filter((row) =>
        ["expired", "changes", "rejected"].includes(row.key),
      ).length,
    };
  }, [currentRecords]);
      ...
      <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
        <View style={styles.hero}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.subtitle}>
              Your requirements and upload fields come from the same Training
              Configuration used by TTTracker web.
            </Text>
          </View>

          <Pressable
            style={styles.primary}
            onPress={() => router.push("/training/upload" as Href)}
          >
        ...
        <View style={styles.metrics}>
          ...
          <Metric
            ...
            icon={<ShieldAlert size={18} color="#be123c" />}
          />
        </View>
        ...
        {loading ? (
          ...
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>
              Use Upload to add your first ticket, licence, VOC or certificate.
            </Text>
          </View>
        ) : (
            ...
            return (
              ...
              >
                <View style={{ flex: 1 }}>
                  ...
                  {record.expiry_date ? (
                    <Text style={styles.cardMeta}>
                      Expiry: {formatTrainingDate(record.expiry_date)}
                    </Text>
                  ) : null}
                </View>
...
const styles = StyleSheet.create({
  ...
  pillText: { fontSize: 9, fontWeight: "900", textAlign: "center" },
  empty: {
    alignItems: "center",
    padding: 30,
    backgroundColor: "#fff",
  ...
  emptyTitle: { marginTop: 8, color: "#0f172a", fontWeight: "900" },
  emptyText: {
    marginTop: 5,
    color: "#64748b",
    fontSize: 12,
